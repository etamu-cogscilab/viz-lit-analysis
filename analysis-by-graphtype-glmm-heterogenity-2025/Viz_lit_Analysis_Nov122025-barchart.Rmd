...
read_rt_summary <- read_rt %>%
  ...
  summarize(meanType1 = mean(Type1, na.rm = T),
            sdType1 = sd(Type1, na.rm = T),
            meanType2 = mean(Type2, na.rm = T),
            sdType2 = sd(Type2, na.rm = T),
            meanType3 = mean(Type3, na.rm=T),
            sdType3 = sd(Type3, na.rm = T))
...
# Create a named list for the contrast matrix
contrast_matrix <- list(
    VR_vs_Monitors_Collapsed = contrast_coefficients
)

comparison_results <- contrast(emm_main_condition, 
                               method = contrast_matrix)
summary(comparison_results, level = 0.95)
```

```{r}
contrast_coefficients <- c(-0.5, 1, -0.5)

# Create a named list for the contrast matrix
contrast_matrix <- list(
    VRMonitor_vs_Others_Collapsed = contrast_coefficients
)
